...
static inline void gpio_set_on_raw(unsigned pin) {
  static volatile unsigned *gpio_set0 = (void *)(0x20200000 + 0x1C);
  
  *gpio_set0 = 1 << pin; // basically put32_raw
}
static inline void gpio_set_off_raw(unsigned pin) {
  static volatile unsigned *gpio_clr0 = (void *)(0x20200000 + 0x28);

  *gpio_clr0 = 1 << pin; // basically put32_raw
}
...
static inline void write_1(unsigned pin, unsigned ncycles) {
    // use gpio_set_on_raw
    unsigned const start = cycle_cnt_read() - compensation;
    gpio_set_on_raw(pin);
